...
http.createServer(function(request, response) {
  ...
  var basePath =  __dirname + '/../template/';

  if (parsedUrl.pathname.indexOf('jscoverage.json') >= 0 || parsedUrl.pathname.indexOf('coverage.xml') >= 0)
    basePath = process.argv[2];

  var filename = path.join(basePath, parsedUrl.pathname);
